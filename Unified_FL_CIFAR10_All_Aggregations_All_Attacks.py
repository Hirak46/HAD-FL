...
RESULTS_DIR = os.path.abspath(os.path.join(os.getcwd(), '..', 'Results_CIFAR-10'))
...
train_data_raw = TorchvisionCIFAR10(root=dataset_dir, train=True, download=True)
test_data_raw = TorchvisionCIFAR10(root=dataset_dir, train=False, download=True)
...
class CIFAR10Dataset(Dataset):
    """Pre-caches all transformed CIFAR-10 samples as tensors in memory."""
...
train_dataset = CIFAR10Dataset(train_data_raw, transform=train_transform)
test_dataset = CIFAR10Dataset(test_data_raw, transform=test_transform)
...
dist_df.to_csv(os.path.join(RESULTS_DIR, 'cifar10_federated_data_distribution.csv'), index=False)
...
print(f"Distribution saved to: {RESULTS_DIR}/cifar10_federated_data_distribution.csv")
...
def create_model():
    """Create MobileNetV2 model adapted for CIFAR-10 (32x32 RGB images, 10 classes)."""
...
print("=" * 80)
print(f"MODEL: MobileNetV2 for CIFAR-10")
print(f"  Total parameters:     {total_params:>12,}")
print(f"  Trainable parameters: {trainable_params:>12,}")
print(f"  Input: 3x32x32 RGB | Output: {NUM_CLASSES} classes")
print(f"  First conv stride=1 (adapted for 32x32 images)")
print("=" * 80)
del _model
...
print("\n" + "=" * 100)
print("COMPREHENSIVE RESULTS SUMMARY — CIFAR-10")
print("=" * 100)
