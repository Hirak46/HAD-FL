...
RESULTS_DIR = os.path.abspath(os.path.join(os.getcwd(), '..', 'Results_MNIST'))
...
train_data_raw = TorchvisionMNIST(root=dataset_dir, train=True, download=True)
test_data_raw = TorchvisionMNIST(root=dataset_dir, train=False, download=True)
...
class MNISTDataset(Dataset):
    """Pre-caches all transformed MNIST samples as tensors in memory."""
...
train_dataset = MNISTDataset(train_data_raw, transform=train_transform)
test_dataset = MNISTDataset(test_data_raw, transform=test_transform)
...
dist_df.to_csv(os.path.join(RESULTS_DIR, 'mnist_federated_data_distribution.csv'), index=False)
...
print(f"Distribution saved to: {RESULTS_DIR}/mnist_federated_data_distribution.csv")
...
class FourLayerCNN(nn.Module):
    """
    4-Layer CNN with BatchNorm for MNIST (1x28x28 grayscale images, 10 classes).
    Architecture:
...
def create_model():
    """Create 4-Layer CNN with BatchNorm for MNIST."""
...
print("=" * 80)
print(f"MODEL: 4-Layer CNN with BatchNorm for MNIST")
...
print("\n" + "=" * 100)
print("COMPREHENSIVE RESULTS SUMMARY — MNIST")
print("=" * 100)
